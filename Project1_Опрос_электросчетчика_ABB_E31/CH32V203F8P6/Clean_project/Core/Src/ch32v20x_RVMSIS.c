...
void RVMSIS_RCC_SystemClock_144MHz(void) {
    ...
    SET_BIT(EXTEN->EXTEN_CTR, EXTEN_PLL_HSI_PRE);//Уберем делитель на 2 с HSI перед PLLSRC
    CLEAR_BIT(RCC->CFGR0, RCC_PLLSRC); //В качестве входного сигнала для PLL выберем HSI
...
void RVMSIS_TIM3_init(void) {
    ...
    SET_BIT(TIM3->CTLR1, TIM_OPM); //One pulse mode off(Счетчик не останавливается при обновлении)
...
void RVMSIS_USART2_Init(void) {
    ...
    CLEAR_BIT(USART2->CTLR1, USART_CTLR1_IDLEIE); //Прерывание по флагу IDLE выключено
...
bool ModbusRTU_Transmit(USART_TypeDef* USART, uint8_t* data, uint16_t Size, uint32_t Timeout_ms) {
    MODBUSRTU_TRANSMIT;
    for (uint16_t i = 0; i < Size; i++) {
        Timeout_counter_ms = Timeout_ms;
        //Ждем, пока линия не освободится
    ...
    Timeout_counter_ms = Timeout_ms;
    while (READ_BIT(USART->STATR, USART_STATR_TC) == 0) {
        if (!Timeout_counter_ms) {
            MODBUSRTU_RECEIEVE;
            return false;
        }
